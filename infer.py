...
def parse_args():
    ...
    # ================= 核心必填路径 =================
    parser.add_argument('--model_path', type=str, default='outputs/tfpp/maize_data/models/best_model.pth',
                        help='训练好的模型文件路径 (.pth)')
    parser.add_argument('--scaler_path', type=str, default='outputs/tfpp/maize_data/scalers/scaler.pkl',
                        help='归一化器文件路径 (.pkl)')
    ...
    parser.add_argument('--output', type=str,
                        default=f'outputs/infer/inference_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
                        help='预测结果保存路径')
